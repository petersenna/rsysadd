...
// written by m0nad in [github.com/m0nad/Diamorphine/blob/master/diamorphine.c]
...
static int __init sys_give_root_init(void){
	...
	if(sysnum < 0){
		printk(KERN_INFO "[sys_give_root] not registered\n");
		return -1;
	}

	printk(KERN_INFO "[sys_give_root] --> sys_call_table[%d]\n", sysnum);
	return 0;
}
